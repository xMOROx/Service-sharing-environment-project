...
import (
	...
	// Tu będzie poprawiony import po wygenerowaniu z .proto
...
func (s *InventoryServer) GetProductInfo(ctx context.Context, req *pb.ProductId) (*pb.ProductInfo, error) {
    start := time.Now()
    defer func() {
        // Rejestrujemy jednorazowo liczbę wywołań i czas trwania
...
// SubscribeLowStockAlerts wysyła alerty co pewien czas, gdy AvailableQuantity <= prog
...
// InteractiveOrderStock to RPC bidirectional streaming (dlatego obie strony recv/send)
